...
OUT    = ROOT / "halal-patrimoine-v2.pdf"
...
def build():
    doc = SimpleDocTemplate(
        ...
        title="Halal & patrimoine v2 — Le guide qui te fait passer de zero a un vrai portefeuille",
        author="L'équipe muslimfinance.net",
        subject="Investissement halal AAOIFI — cas concret d'un portefeuille personnel avec +18 728 EUR de plus-value latente — 2026",
    )
    story = []
    ...
    cover_inner = [
        ...
        [Paragraph(
            "Comment j'ai construit un portefeuille halal<br/>"
            "avec +18 728 EUR de plus-value latente en 2 ans",
            cover_sub)],
        [Spacer(1, 14)],
        [HRFlowable(width=W-48, thickness=1.5, color=GOLD, spaceAfter=24)],
        [Spacer(1, 8)],
        [Paragraph(
            "Cas concret d'un portefeuille perso Trade Republic • "
            "66 NASDAQ halal • 226 S&amp;P 500 halal • Backtests AAOIFI • "
            "Financer hadj &amp; mariage • Or, SCPI, zakat, transmission",
            ps("cv_features",
    ...
    story.append(Paragraph("Avant-propos — Ce que la v2 change", section_head))
    story.append(gold_rule())
    ...
    story.append(Paragraph(
        ...
        body))
    story.append(Paragraph(
        "<b>Deux.</b> Le hadj, le mariage, l'achat d'un logement — ce sont des objectifs "
        "financiers concrets que la communauté musulmane repousse souvent par manque "
        "d'outils halal. Ce guide te donne la méthode pour transformer un salaire ordinaire "
        "en projet finançable en 3 à 7 ans, sans un centime de crédit riba.",
        body))
    story.append(Paragraph(
        "Ce guide n'est pas une formation, un cours vidéo, ni un accès VIP. "
        "C'est un manuel d'auto-formation avec des chiffres réels, des critères vérifiables "
        "et des tableaux actionnables. Pas de théologie de surface, pas de guru.",
        body))
    story.append(Paragraph(
    ...
    toc_items = [
        ...
        ("10","Financer ton hadj, ton mariage, ta maison sans crédit riba",
               "3 objectifs concrets — plans de 3, 5 et 7 ans — tableaux d'épargne halal"),
        ("11","Transmettre : héritage, donation, zakat",
    ...
    story.append(Paragraph(
        ...
        "Et les semi-conducteurs — NVIDIA, Broadcom, AMD, ASML, TSMC, ARM — sont "
        "majoritairement dans le secteur le plus représenté du portefeuille NASDAQ halal.",
        body))
    story.append(Paragraph(
    ...
    story.append(Paragraph("Ma preuve — mon portefeuille tech halal (Trade Republic)", section_head))
    story.append(Paragraph(
        "Le tableau ci-dessous n'est pas une simulation, c'est mon compte réel. "
        "Chaque ligne est une position ouverte, chaque montant est une plus-value latente "
        "au jour de la capture. Toutes les positions sont AAOIFI-compliant "
        "(revenus non conformes inférieurs à 5 %, ratio dette conforme).",
        body))

    _ttp = ps("_ttp", fontName="Helvetica", fontSize=9, leading=13, textColor=DARK)
    _ttph = ps("_ttph", fontName="Helvetica-Bold", fontSize=9, leading=13, textColor=IVORY)
    portfolio_perso = [
        [Paragraph("Position", _ttph), Paragraph("Valeur", _ttph), Paragraph("Plus-value latente", _ttph)],
        [Paragraph("NVIDIA", _ttp),       Paragraph("10 915 EUR", _ttp), Paragraph("+2 492 EUR", _ttp)],
        [Paragraph("Advantest", _ttp),    Paragraph("5 450 EUR",  _ttp), Paragraph("+1 425 EUR", _ttp)],
        [Paragraph("ARM (ADR)", _ttp),    Paragraph("4 297 EUR",  _ttp), Paragraph("+2 821 EUR", _ttp)],
        [Paragraph("ASML", _ttp),         Paragraph("3 432 EUR",  _ttp), Paragraph("+1 764 EUR", _ttp)],
        [Paragraph("AMD", _ttp),          Paragraph("3 246 EUR",  _ttp), Paragraph("+1 985 EUR", _ttp)],
        [Paragraph("TSMC (ADR)", _ttp),   Paragraph("2 751 EUR",  _ttp), Paragraph("+1 421 EUR", _ttp)],
        [Paragraph("Micron Technology", _ttp), Paragraph("2 728 EUR", _ttp), Paragraph("+1 792 EUR", _ttp)],
        [Paragraph("Broadcom", _ttp),     Paragraph("1 846 EUR",  _ttp), Paragraph("+1 095 EUR", _ttp)],
        [Paragraph("Apple", _ttp),        Paragraph("1 873 EUR",  _ttp), Paragraph("+641 EUR",   _ttp)],
        [Paragraph("Comfort Systems USA", _ttp), Paragraph("1 543 EUR", _ttp), Paragraph("+1 236 EUR", _ttp)],
        [Paragraph("<b>Sous-total tech halal</b>", _ttph), Paragraph("<b>~38 100 EUR</b>", _ttph), Paragraph("<b>+16 700 EUR</b>", _ttph)],
    ]
    tp = Table(portfolio_perso, colWidths=[7*cm, 4*cm, 5.5*cm], repeatRows=1)
    tp.setStyle(TableStyle([
        ...
        ("BOTTOMPADDING",(0,0),(-1,-1),  7),
        ("VALIGN",      (0,0), (-1,-1),  "TOP"),
        ("ROWBACKGROUNDS",(0,1),(-1,-2), [IVORY, STONE]),
        ("LINEBELOW",   (0,1), (-1,-3),  0.3, LIGHT_GREY),
    ]))
    story.append(tp)
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "Ces positions constituent la portion tech du portefeuille total. "
        "Le portefeuille complet dépasse <b>49 000 EUR de valorisation</b> pour une "
        "plus-value latente cumulée de <b>+18 728 EUR</b> — soit environ <b>+60 %</b> "
    ...
    story.append(callout_box(
        ...
        "l'obsolescence de ton propre travail, c'est d'en devenir actionnaire."
    ))
    story.append(Paragraph(
        "Précision : les performances passées ne préjugent pas des performances futures. "
        "Ces positions ont bénéficié d'un cycle haussier semi-conducteurs exceptionnel "
        "en 2023-2026. Un cycle baissier est toujours possible. Ne mets jamais sur ces "
        "actions ce dont tu as besoin dans les 3 prochaines années.",
        body_grey))
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # CHAPITRE 10 — Financer ton hadj, ton mariage, ta maison sans crédit riba
    # ══════════════════════════════════════════════════════════════════════════
    chapter_header("10",
        "Financer ton hadj, ton mariage, ta maison sans crédit riba",
        "Trois objectifs concrets. Trois plans d'épargne halal, sur 3 à 7 ans.",
        story)

    story.append(Paragraph(
        "Ce chapitre transforme le portefeuille en outil de financement d'objectifs "
        "personnels. Trois objectifs typiques : le hadj (voyage pour La Mecque), "
        "le mariage (mahr + fête), l'apport pour un logement. "
        "Chaque objectif a un coût, une échéance, et donc une mensualité d'épargne "
        "à combiner avec un rendement halal réaliste.",
        body))

    story.append(Paragraph("Objectif 1 — Le hadj (3 à 5 ans)", section_head))
    story.append(Paragraph(
        "Coût moyen d'un hadj depuis la France en 2026 : 7 000 à 10 000 EUR par personne "
        "(agrée agence agréée, formule standard 20 jours). Pour un couple : 14 000 à 20 000 EUR. "
        "Pour partir dans 4 ans, il faut mettre 300 EUR par mois par personne dans un support "
        "halal rentable — pas dans un livret A à 3 %.",
        body))

    _th = ps("_th", fontName="Helvetica", fontSize=9, leading=13, textColor=DARK)
    _thh = ps("_thh", fontName="Helvetica-Bold", fontSize=9, leading=13, textColor=IVORY)
    hadj_data = [
        [Paragraph("Objectif hadj", _thh), Paragraph("Coût cible", _thh), Paragraph("Mensualité 3 ans", _thh), Paragraph("Mensualité 5 ans", _thh)],
        [Paragraph("1 personne — formule standard", _th), Paragraph("8 000 EUR",  _th), Paragraph("200 EUR", _th), Paragraph("115 EUR", _th)],
        [Paragraph("Couple — formule standard", _th),    Paragraph("16 000 EUR", _th), Paragraph("400 EUR", _th), Paragraph("230 EUR", _th)],
        [Paragraph("Couple + parents (4 pers.)", _th),   Paragraph("32 000 EUR", _th), Paragraph("800 EUR", _th), Paragraph("460 EUR", _th)],
    ]
    t_hadj = Table(hadj_data, colWidths=[6*cm, 3*cm, 3.5*cm, 4*cm], repeatRows=1)
    t_hadj.setStyle(TableStyle([
        ("BACKGROUND",  (0,0), (-1,0),   DARK),
        ("LINEBELOW",   (0,0), (-1,0),   2, GOLD),
        ("LEFTPADDING", (0,0), (-1,-1),  8),
        ("RIGHTPADDING",(0,0), (-1,-1),  8),
    ...
    story.append(Paragraph("Objectif 2 — Le mariage (3 à 5 ans)", section_head))
    story.append(Paragraph(
        "Un mariage musulman en France coûte en moyenne 12 000 à 25 000 EUR "
        "(mahr, salle, traiteur halal, tenues, photographe, voyage de noces). "
        "Cette dépense arrive souvent au moment le pire — jeune actif, salaire modeste, "
        "pression familiale. Anticiper 3 à 5 ans à l'avance transforme cette dépense "
        "en projet finançable.",
        body))
    story.append(Paragraph(
        "Deux règles simples pour un mariage halal financé sans crédit :",
        body))
    _mr = [
        "Un compte-titres dédié \"mariage\" (Trade Republic ou DEGIRO) ouvert dès qu'on envisage le projet — même sans partenaire encore identifié",
        "Un virement mensuel automatique le 5 du mois (juste après le salaire), non négociable, sur ETF halal AAOIFI",
        "Le mahr (dot obligatoire) est payé en cash au moment du contrat — pas de crédit, pas de dette matrimoniale, c'est un principe",
        "Zéro anticipation par crédit à la consommation, zéro rachat de crédit : le mariage se prépare, il ne se finance pas à posteriori",
    ]
    for r in _mr:
        story.append(Paragraph(f"- {r}", bullet))

    story.append(Paragraph("Objectif 3 — L'apport pour un logement (5 à 7 ans)", section_head))
    story.append(Paragraph(
        "L'achat immobilier reste le plus gros objectif financier de la vie. "
        "Deux options halal existent en France : payer cash (partiel ou total) et le "
        "financement mourabaha (courtiers spécialisés type 570easi). "
        "Dans les deux cas, il faut un apport significatif — au minimum 25 à 40 % du prix.",
        body))
    story.append(Paragraph(
        "Pour un appartement à 250 000 EUR en Île-de-France, il faut donc entre "
        "62 000 et 100 000 EUR d'apport. En partant de zéro sur 7 ans, cela demande "
        "600 à 1 000 EUR d'épargne mensuelle placée sur ETF halal.",
        body))

    apport_data = [
        [Paragraph("Prix logement", _thh), Paragraph("Apport 30 %", _thh), Paragraph("Mensualité 5 ans", _thh), Paragraph("Mensualité 7 ans", _thh)],
        [Paragraph("150 000 EUR (province)", _th),   Paragraph("45 000 EUR",  _th), Paragraph("600 EUR",   _th), Paragraph("380 EUR", _th)],
        [Paragraph("250 000 EUR (grande ville)", _th),Paragraph("75 000 EUR",  _th), Paragraph("1 000 EUR", _th), Paragraph("635 EUR", _th)],
        [Paragraph("400 000 EUR (Paris/IDF)", _th),  Paragraph("120 000 EUR", _th), Paragraph("1 600 EUR", _th), Paragraph("1 020 EUR", _th)],
    ]
    t_apport = Table(apport_data, colWidths=[5.5*cm, 3.5*cm, 3.5*cm, 4*cm], repeatRows=1)
    ...
    story.append(Spacer(1, 8))
    story.append(callout_box(
        "Règle inverse : si tes revenus ne permettent pas d'épargner ces mensualités, "
        "c'est ton projet immobilier qui doit s'adapter (ville moins chère, surface plus "
        "petite, apport familial), pas l'échéance. Un mauvais achat halal coûte plus "
        "cher qu'un bon achat riba sur 25 ans."
    ))

    story.append(Paragraph("Routine mensuelle simple", section_head))
    story.append(Paragraph(
        "Le 3 du mois : versement salaire. Le 5 du mois : virements automatiques "
        "vers les 3 comptes-titres dédiés (hadj / mariage / apport). "
        "Le reste peut être dépensé sans culpabilité. C'est l'automatisation qui "
        "protège du renoncement — pas la discipline.",
        body))
    story.append(PageBreak())

    # ══════════════════════════════════════════════════════════════════════════
    # CHAPITRE 11 — Transmettre : héritage, donation, zakat
    # ══════════════════════════════════════════════════════════════════════════
    chapter_header("11",
    ...
    back_inner = [
        ...
        [Paragraph(
            "Screener AAOIFI en temps réel — muslimfinance.net",
            ps("bc_feat", fontName="Helvetica", fontSize=11, leading=17,
               textColor=IVORY, spaceAfter=6))],
        [Paragraph(
            "Backtest portefeuille halal vs indices classiques",
            ps("bc_feat2", fontName="Helvetica", fontSize=11, leading=17,
               textColor=IVORY, spaceAfter=6))],
        [Paragraph(
            "Newsletter hebdo : screening à jour + tweets backtests",
            ps("bc_feat3", fontName="Helvetica", fontSize=11, leading=17,
               textColor=IVORY, spaceAfter=6))],
        [Paragraph(
            "Consulting individuel (méthode, pas conseil personnalisé)",
            ps("bc_feat4", fontName="Helvetica", fontSize=11, leading=17,
...
if __name__ == "__main__":
    out = build()
    # Copy to web/public/products/ (v2 filename)
    dest = pathlib.Path.home() / "work/halal-nasdaq/web/public/products/halal-patrimoine-v2.pdf"
