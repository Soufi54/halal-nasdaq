"""
Scrape S&P 500 holdings (tickers + poids) depuis SlickCharts.
Sortie : data/sp500_weights.json
"""
...
OUTPUT = DATA_DIR / "sp500_weights.json"
...
def scrape_sp500() -> list[dict]:
    ...
    if not table:
        ...
        for script in script_tags:
            if script.string and "sp500List" in script.string:
                match = re.search(r"sp500List\s*=\s*(\[.*?\]);", script.string, re.DOTALL)
                if match:
                    return json.loads(match.group(1))
        raise ValueError("Impossible de trouver les donnees S&P 500 sur la page")
...
def main():
    print(f"Scraping S&P 500 depuis {URL}...")
    holdings = scrape_sp500()
